...
import { retrieveItemChunks, type RetrievedChunk } from "@/lib/transcripts/retrieve";
...
export async function POST(
  ...
  try {
    ...
    const transcript = (item.master_transcript ?? "").trim();
    const summary = (item.summary ?? "").trim();

    if (!transcript && !summary) {
      return NextResponse.json({
        reply:
          "There's no master transcript or summary on this item yet. Record something first, then ask again.",
        sources: [],
        grounded: false,
    ...
    try {
      ...
      retrieved = await retrieveItemChunks(
        supabase,
        itemId,
        queryEmbedding,
        RETRIEVAL_TOP_K,
    ...
    if (retrieved.length > 0) {
      ...
      const label =
        transcript.length > 0 ? "Master transcript (truncated)" : "Item summary (truncated)";
    ...
    const systemContent = [
      "You answer questions about the user's voice-note item.",
